...
public class HttpsMockBase {
	...
	public static byte[] decrypt(byte data[], SecureRandom secureRandom)
			throws Exception {
		Cipher cipher = Cipher.getInstance(privateKey.getAlgorithm());
	...
	public static byte[] decrptByDES(byte data[], SecureRandom seed)
			throws Exception {
		if (publicKey == null) {
			publicKey = CertificateUtils.readPublicKeys();
		}
	...
	public static byte[] encrptByDES(byte data[], SecureRandom seed)
			throws Exception {
		if (publicKey == null) {
			publicKey = CertificateUtils.readPublicKeys();
		}
	...
	static String random() {
		...
		for (int i = 0; i < seedLength; i++) {
			sb.append(chars[rd.nextInt(seedLength)]);
		}
		return sb.toString();
	}
}
